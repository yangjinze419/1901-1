// 进入后台首页判断功能-左菜单显示和隐藏等功能
// 注意发送退出请求时-为了后台安全，退出后台界面时要请求后台接口判断是否含有Session若没有返给前端信息提醒,不要只做前端操作单方面的退出
...
$('[data-logout]').on('click',function(){
    ...
    $('#logoutModal').off('click','.btn-primary').on('click','.btn-primary',function(){
        /*6.发送退出请求-为了后台安全，退出后台界面时要请求后台接口判断是否含有Session若没有返给前端信息提醒,不要只做前端操作单方面的退出*/
        $.ajax({
            type:'get',
